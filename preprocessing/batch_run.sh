#!/bin/bash
# ============================================================
#  批量运行：自动跳过已完成的被试，支持断点续跑
#  用法: bash batch_run.sh <OUT_ROOT> [并行数, 默认4]
#
#  输出文件 (全部在 OUT_ROOT 下)：
#    pipeline_all.log   — 所有被试的详细运行日志 (实时)
#    master_log.tsv     — 每个被试一行的汇总表
#    all_results.csv    — 成功被试的数值结果
# ============================================================
...
for id in "${all_ids[@]}"; do
    result="${OUT_ROOT}/${id}/neurochem_line.txt"
    if [ -f "$result" ]; then
...
# --- 并行执行 ---
printf '%s\n' "${todo_ids[@]}" | \
    xargs -P ${NJOBS} -I {} bash "${SCRIPT_DIR}/run_lesion_to_mni.sh" {} "${OUT_ROOT}"

# --- 汇总报告 ---
echo ""
echo "============ 汇总报告 ============"
total=$(ls -d ${OUT_ROOT}/*/ 2>/dev/null | wc -l)
success=$(grep -rL "MISSING\|FAILED" ${OUT_ROOT}/*/neurochem_line.txt 2>/dev/null | wc -l)
input_miss=$(grep -rl "INPUT_MISSING" ${OUT_ROOT}/*/neurochem_line.txt 2>/dev/null | wc -l)
failed=$(grep -rl "FAILED" ${OUT_ROOT}/*/neurochem_line.txt 2>/dev/null | wc -l)
no_result=$(for d in ${OUT_ROOT}/*/; do [ ! -f "$d/neurochem_line.txt" ] && echo "$d"; done | wc -l)
...
if [ "$failed" -gt 0 ]; then
    echo "--- 失败的被试 ---"
    grep -rl "FAILED" ${OUT_ROOT}/*/neurochem_line.txt | while read f; do
...
# --- 生成 master_log.tsv：从全局日志提取每个被试的状态 ---
...
for d in ${OUT_ROOT}/*/; do
    [ ! -d "$d" ] && continue
    id=$(basename "$d")
    result="$d/neurochem_line.txt"

    if [ ! -f "$result" ]; then
        echo -e "${id}\t无结果\t-\t未生成neurochem_line.txt" >> "${MASTER_LOG}"
...
for i in $(seq 1 26); do header="${header},label_${i}_vol"; done
echo "${header}" > "${OUT_ROOT}/all_results.csv"
grep -rL "MISSING\|FAILED" ${OUT_ROOT}/*/neurochem_line.txt 2>/dev/null | \
